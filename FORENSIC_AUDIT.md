...
These findings are about the Bruce firmware ecosystem as a whole, not any single hardware variant. The device I was working on was just the door I walked through.
...
```
The board configuration doesn't define `DEEPSLEEP_WAKEUP_PIN`. The default from `include/precompiler_flags.h`:
```cpp
...
This affects every device running Bruce firmware or the bmorcelli launcher. Every user who thinks their device is off is wrong.
...
4. Deep Sleep broken on all hardware (DEEPSLEEP_WAKEUP_PIN=-1 by default)
...
- Deep Sleep broken on all hardware (DEEPSLEEP_WAKEUP_PIN=-1 by default)
